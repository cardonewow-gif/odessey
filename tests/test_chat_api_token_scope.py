...
REMOTE_DEV_BASH_GUARD = "_require_remote_development_scope_for_bash"
...
def _load_remote_dev_bash_guard():
    tree = _source_tree()
    helper = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef) and node.name == REMOTE_DEV_BASH_GUARD
    )
    module = ast.Module(body=[helper], type_ignores=[])
    ast.fix_missing_locations(module)
    namespace = {
    ...
    exec(compile(module, str(CHAT_ROUTES), "exec"), namespace)
    return namespace[REMOTE_DEV_BASH_GUARD]
...
def test_remote_dev_bash_guard_allows_non_bash_requests():
    guard = _load_remote_dev_bash_guard()

    guard(_request(api_token=True, scopes=["chat"]), "false")


def test_remote_dev_bash_guard_allows_remote_development_bearer_tokens():
    guard = _load_remote_dev_bash_guard()

    guard(_request(api_token=True, scopes=["chat", "remote_development"]), "true")


@pytest.mark.parametrize("scopes", [None, [], ["chat"], ["documents:read", "chat"]])
def test_remote_dev_bash_guard_rejects_missing_remote_development(scopes):
    guard = _load_remote_dev_bash_guard()
    ...
    assert "remote development" in str(exc.value.detail)
