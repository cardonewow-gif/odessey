...
# then open http://<lan-ip>:7860 on the same Wi-Fi
```
...
For the Expo/iPhone companion over Tailscale, prefer a private HTTPS origin instead of a raw Tailscale IP:

```bash
./scripts/setup-companion-tailscale.sh
# restart Odysseus if it is already running, then mint a fresh QR from:
# http://127.0.0.1:7860/api/companion/pair
```

That helper configures `tailscale serve` for `http://127.0.0.1:7860`, writes
`COMPANION_BASE_URL=https://<machine>.ts.net` into `.env`, and makes the pairing
QR advertise the private `ts.net` origin the mobile app already prefers.
...
For the mobile companion over Tailscale, keep Odysseus on loopback and let
Tailscale terminate private HTTPS for you:

1. Install Tailscale on the Mac and sign into your tailnet.
2. Run:
   ```bash
   ./scripts/setup-companion-tailscale.sh
   ```
3. Restart Odysseus so it reloads `.env`, then mint a fresh pairing code from
   `http://127.0.0.1:7860/api/companion/pair`.

That path keeps the raw Odysseus app port private, publishes a trusted
`https://<machine>.ts.net` origin inside the tailnet, and makes the QR payload
include `base_url` for the mobile app.

If you need direct HTTPS on a LAN IP without Tailscale, use local certificates
instead:

1. Change the bind address to `0.0.0.0` in `.env` (`APP_BIND=0.0.0.0` or `ODYSSEUS_HOST=0.0.0.0`).
2. Generate a locally-trusted cert for your LAN IPs using [mkcert](https://github.com/FiloSottile/mkcert):
   ```bash
   mkcert -install
...
4. Install the `mkcert` CA on any other device you want to access Odysseus from
   (for iOS, install the profile and trust it in Certificate Trust Settings).
